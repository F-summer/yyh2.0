...
Page({
      ...
      if (startTime < 0) {
        ...
        if (endTime < 0) {
          ...
          that.data.baginTime = setInterval(function() {
            ...
            that.setData({
              timeText: '距点赞活动结束 剩余：' + timeover
            });
          }, 1000);
          that.data.noPiao = false;
        } else {
          that.setData({
            timeText: '点赞活动已经结束，感谢您的支持！'
          });
          that.data.noPiao = true;
        }
  ...
  onLoad: function(options) {
    wx.getImageInfo({ //  小程序获取图片信息API
      src: 'https://www.yaobc.info/images/yyh/card2.png',
      success: function (res) {
        wx.setStorageSync('card_img', res.path);
      },
      fail(err) {
        console.log(err)
      }
    ...
    wx.getImageInfo({ //  小程序获取图片信息API
      src: 'https://www.yaobc.info/images/yyh/votelogo2.jpg',
      success: function (res) {
        wx.setStorageSync('votelogo', res.path);
      },
      fail(err) {
        console.log(err)
      }
